    ...
    def evaluate(self, data, dims=None, overall=True, print_result=False):
        ...
        for dim in eval_dims:
            ...
            if dim == "consistency" or dim == "fluency":
                ...
                sent_score = self.scorer.score(input_list, batch_size=1)
...
class D2tEvaluator:
    ...
    def evaluate(self, data, dims=None, overall=True, print_result=False):
        ...
        for dim in eval_dims:
            ...
            input_list = add_question(
                dimension=dim, output=output_list, ref=ref_list, task=self.task
            )
            score = self.scorer.score(input_list)
    ...
    def evaluate(self, data, print_result=False):
        ...
        for i in range(n_data):
            ...
            for j in range(len(system_outputs)):
                ...
                output_list.append(system_outputs[j])
        input_list = add_question(
            dimension=self.dim, output=output_list, src=src_list, task=self.task
        )
        sent_score = self.scorer.score(input_list)
...
def get_evaluator(task, max_length=1024, device="cuda:0", cache_dir=None):
    ...
    if task == "summarization":
        return SumEvaluator(max_length=max_length, device=device, cache_dir=cache_dir)
    elif task == "dialogue":
        return DialogEvaluator(
            max_length=max_length, device=device, cache_dir=cache_dir
        )
    elif task == "data2text":
        return D2tEvaluator(max_length=max_length, device=device, cache_dir=cache_dir)
    elif task == "fact":
        return FactEvaluator(max_length=max_length, device=device, cache_dir=cache_dir)
    else:
        raise NotImplementedError(
